...
type ApiReplyWrapper = FastifyPluginCallback<ReplyWrapperNS.ReplyWrapperOptions>;
...
function replyWrapper(...[fastify, opts, pluginCb] : Parameters<ApiReplyWrapper>) {
  try {
    ...
    fastify.addHook('preSerialization', (request: FastifyRequest, reply: FastifyReply, payload: any, done:any) => {
      ...
      if (ALLOW_EMPTY_API_CODE) {
        return done(err, {
          status: 'success', code: 'SUCCESS', message: 'Success.', data: payload,
        });
      }
